...
export function assertBoxValid(props: BoxProps): true {
  ...
  // exact case Ink's colorize.ts throws on) is caught by solid-tui's error
  // boundary, not the post-flush paint pass where a throw wedges Solid's
  ...
  // rather than throwing (a throw in the post-flush paint pass wedges Solid's
  ...
  // solid-tui's error boundary (onErrorCaptured → ErrorOverview), exactly like
  ...
  // through Solid's post-flush commit and wedge its scheduler. A falsy value
  ...
  // 'repeat')` in the post-flush PAINT pass — wedging Solid's scheduler. Resolving
