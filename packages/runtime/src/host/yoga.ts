...
export function attachYoga(node: YogaCarrier): void {
  ...
  // are passed through Solid's reactive system (which may include undefined
...
export function insertYogaChild(parent: TuiContainer, child: TuiNode, _domIndex: number): void {
  ...
  const childYoga = child.yoga as YogaNode;
  const currentYogaParent = childYoga.getParent();
  if (currentYogaParent) {
    currentYogaParent.removeChild(childYoga);
  }
  const yIdx = yogaIndexFor(parent, child);
  (parent.yoga as YogaNode).insertChild(childYoga, yIdx);
}
...
const YOGA_PROP_SETTERS: Record<string, (n: YogaNode, v: unknown) => void> = {
  ...
  //     setFlexBasisAuto): Solid's [Number, String] prop validation only WARNS on a
  ...
  //   else   → setFlexBasisAuto()  — this is the load-bearing fallback: Solid's
  ...
  // default DISPLAY_FLEX — Solid can't tell `display={undefined}` from an omitted prop, so
  ...
  //     max", the G19 removal reset) because Solid's host renderer can deliver raw
  ...
  //     equivalent of Ink's else with an absent value (Solid can deliver raw null,
...
// margin/padding are number-only; numeric STRINGS are still accepted because Solid
...
export function applyYogaProp(
  ...
  if (!setter) return;
  // Solid calls patchProp with `undefined` for every declared prop a user
  ...
  //   1. Solid calls patchProp(el, key, null, undefined) for every declared prop
  //      that is absent on the first mount (old=null, new=undefined).
  //   2. Solid calls patchProp(el, key, null, undefined) for props absent in a
  ...
  //
  // Blocker 2: Solid's HOST renderer passes next=null (not undefined) when a key
