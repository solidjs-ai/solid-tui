import { type RendererOptions } from "../solid-universal.ts";
import {
  createBox,
  createStatic,
  createText,
...
function setStoredMouseHandler(node: TuiNode, key: MouseHandlerName, handler: unknown): void {
  ...
  if (typeof handler === "function") handlers[key] = handler as never;
...
export function buildNodeOps(options: TtyRendererOptions): RendererOptions<TuiNode> {
  ...
  function createElement(type: string): TuiNode {
    switch (type) {
      ...
      default:
        throw new Error(`Unknown solid-tui element type: ${type}`);
    }
  }
  ...
  function setText(node: TuiNode, text: string): void {
    ...
    // An empty text-leaf can mount as a Solid fragment anchor (insert() exempts empty
    ...
    // is that solid-tui attaches a yoga node to EVERY <Transform> (even inline ones
  ...
  function insert(child: TuiNode, parent: TuiNode, anchor: TuiNode | null): void {
    ...
    // Move semantics: if the child is already mounted (Solid's keyed reorder
  ...
  function patchProp(el: TuiNode, key: string, prev: unknown, next: unknown): void {
    if (isMouseHandlerProp(key)) {
      if (el.type === "tui-box" || el.type === "tui-text" || el.type === "tui-virtual-text") {
        ...
        else setStoredMouseHandler(el, key, next);
      }
      onCommit();
      return;
    }
    ...
    if (
      ...
      if (isYogaProp(key)) {
        ...
        // Handled at the Solid component level (box.ts / text.ts / transform.ts),
        // not stored on the DOM node. Silently ignore so we don't warn.
      } else if (key === "key" || key === "ref" || key.startsWith("on")) {
        // Reserved by Solid / event keys, ignore.
      } else if (process.env["NODE_ENV"] !== "production") {
        // eslint-disable-next-line no-console
        console.warn(`[solid-tui] unknown prop "${key}" on <${el.type}>`);
      }
      onCommit();
      return;
    }
  ...
  const nodeOps: RendererOptions<TuiNode> = {
    createElement,
    createTextNode,
    replaceText: setText,
    ...
    getParentNode: (node) => parentNode(node) ?? undefined,
    getFirstChild: (node) => (isContainer(node) ? node.children[0] : undefined),
