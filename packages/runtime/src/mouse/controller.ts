...
import type { TuiContainer, TuiNode } from "../host/nodes.ts";
...
const INLINE_MOUSE_WARNING =
  "[solid-tui] Mouse handlers only fire in fullscreen mode. Use app.mount({ fullscreen: true }) for targeted element mouse events, or useMouseInput() for raw inline mouse input.";
...
export function createMouseController(options: CreateMouseControllerOptions): MouseController {
  ...
  let activeDrag:
    | { node: TuiNode; registration: DraggableRegistration; moved: boolean }
    | undefined;
  ...
  function onRawMouse(raw: SgrMouseEvent) {
    ...
    if (raw.type === "down") pressedButtons.add(raw.button);
    else if (raw.type === "up") pressedButtons.delete(raw.button);
    ...
    if (raw.type === "down") {
      ...
      if (draggable) {
        ...
        activeDrag = { ...draggable, moved: false };
      }
      return;
    }
  ...
  function makeMouseEvent(
    ...
    const event = {
      ...
      screenX: raw.x - 1,
      screenY: raw.y - 1,
      target: getMouseTarget(targetNode),
      currentTarget: null,
      stopPropagation() {
        stopped = true;
      },
  ...
  function dispatchMouseEvent(
    ...
    while (current) {
      ...
      if (handler) {
        handler(withCurrentTarget(event, getMouseTarget(current)));
        if (stopped()) return;
      }
  ...
  function makeDragEvent(
    ...
    return withCurrentTarget(event, getMouseTarget(node));
  }
  ...
  function removeNode(node: TuiNode) {
    ...
    if (capturedNode === node) {
      activeDrag?.registration.onCancel?.();
      capturedNode = undefined;
      activeDrag = undefined;
    }
