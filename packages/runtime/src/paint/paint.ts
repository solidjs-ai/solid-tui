    ...
    for (const op of this.ops) {
      ...
      for (let [index, line] of lines.entries()) {
        ...
        if (clipH) {
          ...
          // glyph's extra column — that produced a solid-tui-specific leading space
...
// children string — the solid-tui equivalent of Ink's `internal_transform` being
...
// childNode, so it never advances `index`; Solid, by contrast, materializes those
...
function squashInlineChildren(children: readonly TuiNode[], inheritedBg: unknown): string {
  ...
  for (const child of children) {
    ...
    // Comments (Solid's null/v-if/false renders) and EMPTY text-leaves (`{''}` /
...
function squashTransformChild(child: TuiNode, index: number, inheritedBg: unknown): string {
  ...
  if (child.type === "tui-transform") {
    ...
    // positional index must skip Solid comment nodes (null/v-if/false renders),
...
function drawBorder(
  ...
  // borderStyle string during render and throws there (caught by solid-tui's error
  // boundary), so paint never sees an invalid name. A raw throw HERE would unwind
  // through Solid's post-flush commit and wedge its internal flush state. (audit 2.3)
  if (!chars) return;
...
function recordVirtualTextHits(
  ...
  for (const span of spans) {
    ...
    recordHit(
      hitMap,
      span.node,
      { x: x + start.column, y: y + start.line, width: rectWidth, height },
      clip,
    );
  }
}
