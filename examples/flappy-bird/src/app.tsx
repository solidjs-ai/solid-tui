import { Index, createMemo, onCleanup } from "solid-js";
import { createStore, produce, reconcile } from "solid-js/store";
import chalk from "chalk";
import { Box, Text, useApp, useInput } from "@solid-tui/runtime";
...
export default function App() {
  const { exit } = useApp();
  const [world, setWorld] = createStore<World>(makeWorld(0));

  function flap(): void {
    if (world.dead) return;
    setWorld("started", true);
    setWorld("bird", "vy", FLAP_VELOCITY);
  }

  function restart(): void {
    setWorld(reconcile(makeWorld(world.best)));
  }
  ...
  useInput((input, key) => {
    ...
    if (input === " " || input === "w" || key.upArrow) flap();
  });

  const tickId = setInterval(() => {
    if (world.dead || !world.started) return;
    setWorld(produce(step));
  }, TICK_MS);
  onCleanup(() => clearInterval(tickId));

  const lines = createMemo(() => renderFrame(world));
  ...
  return (
    <Box flexDirection="column">
      <Box borderStyle="round" borderColor="cyan" flexDirection="column" width={playfieldWidth}>
        <Index each={lines()}>{(line) => <Text wrap="truncate">{line()}</Text>}</Index>
      </Box>
      <Box flexDirection="row" justifyContent="space-between" width={playfieldWidth} paddingX={1}>
        <Text color="yellow" bold>
          score {world.score} best {world.best}
        </Text>
